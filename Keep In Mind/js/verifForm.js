...
$(document).ready(function(){
	C1 = document.getElementById("usernameConnex");
	C2 = document.getElementById("passwordConnex");
...
function lancement(){
	...
	function verification(){
		if (verifUsernameConnex() && verifPasswordConnex()) {
			document.getElementById("boutonConnex").disabled=false;
		}
		else{
			document.getElementById("boutonConnex").disabled=true;
		}
	...
	function verifExistanceUsername(){
		...
		while (!bool && i < tabUsername.length){
			if (username.value == tabUsername[i][0]) {
				bool = true;
			}
			i++;
		}
